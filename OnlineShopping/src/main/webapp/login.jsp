
<!DOCTYPE html>
<html>
<head>
<link rel="stylesheet" href="css/signup-style.css">
<title>Login</title>
</head>
<body>
<div id='container'>
  <div class='signup'>
     <form action="loginAction.jsp" method="post">
     <input type="email" name="email" placeholder="Enter Email" required>     
     <input type="password" name="password" placeholder="Enter Password" required>
     <input type="submit" value="login">    
     </form>
      <h2><a href="signup.jsp">SignUp</a></h2>
       <h2><a href="forgotPassword.jsp">Forgot Password?</a></h2>
  </div>
  <div class='whysignLogin'>
  
  <%
  String msg = request.getParameter("msg");
  if("notexist".equals(msg)){  
  %>
  <h1>Incorrect Username or Password</h1>
  <%}%>
  
  <%if("invalid".equals(msg)) 
  {%>
<h1>Some thing Went Wrong! Try Again !</h1>
  <%}%>	
   <img src="img/bestbuylogo.jpg"width="120" height="120">
    
   
    <p>
Welcome to <b>BestBuy.com</b>, your one-stop destination for all things fabulous! Dive into a world of fashion, beauty, home decor, and more, curated just for you.

Discover the latest trends and timeless classics in clothing, accessories, and lifestyle products, all at your fingertips. Whether you're searching for the perfect outfit, a unique gift, or simply indulging in a little self-care, we've got you covered.</p>
  </div>
</div>

</body>
</html>